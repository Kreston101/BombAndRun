...
{
    public GameObject tree, plane, landingPlane;
    ...
    void Start()
    {
        ...
        //loop through the spawn points and spawn trees, then add them to the obstacles list
        for (int i = 0; i <= spawnPoints.Length - 1; i++)
        {
            obstacles.Add(Instantiate(tree, spawnPoints[i].transform.position, transform.rotation));
        }
        ...
        //starts the level with a 3 second delay, gives the player time to prepare
        StartCoroutine(GetReady());
    }

    void Update()
    {
        //checks is the level has been cleared
